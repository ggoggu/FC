...
* **핵심 아키텍처 원칙**: 
  ...
  - **Gameplay Ability System (GAS)** 전면 적용: 모든 전투, 스킬, 버프/디버프, 스태미나를 GAS(`UAbilitySystemComponent`, `UAttributeSet`)로 관리.
  - **Fast Array Serialization**: 동적 아이템 및 무기 목록은 `FFastArraySerializer`를 필수 적용하여 대역폭 최적화.
...
│   (최상위 아키텍처 및 파이프라인 지침)       │ │   (ue5-harness MCP 서버)                │
│ • .antigravity/rules/                        │ │ • Scripts/                              │
...
Antigravity는 프로젝트 로드 시 아래 규칙들을 자동으로 참조하여 코드를 작성합니다:

| 문서 위치 | 역할 및 통제 영역 |
| :--- | :--- |
| [**`AGENTS.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/AGENTS.md) | 프로젝트 전체 최상위 지침: 4계층 권한 분리 모델, GAS 규칙, FastArray 의무화, 모듈형 파이프라인 프로토콜 정의 |
| [**`01-ue58-coding-standards.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/.antigravity/rules/01-ue58-coding-standards.md) | UE5.8 최신 C++ 코딩 표준 (`TObjectPtr`, IWYU, PascalCase, 스마트 포인터 규칙) |
| [**`02-multiplayer-conventions.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/.antigravity/rules/02-multiplayer-conventions.md) | 서버 권한(`HasAuthority`), Client-Prediction, UI 완벽 분리(UI는 Slate/UMG 클라이언트 전용) |
| [**`03-loop-engineering.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/.antigravity/rules/03-loop-engineering.md) | 자율 자가 교정(Self-Healing) 루프, 에러 파싱 트리아지 가이드 (최대 5회 자가 수정) |
| [**`04-gameplay-ability-system.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/.antigravity/rules/04-gameplay-ability-system.md) | GAS 어트리뷰트 선언 매크로(`ATTRIBUTE_ACCESSORS`), RepNotify, Gameplay Cue 사용 규칙 |
| [**`05-combat-and-inventory-networking.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/.antigravity/rules/05-combat-and-inventory-networking.md) | `FFastArraySerializer` 구현 패턴, `COND_OwnerOnly` / `COND_SkipOwner` 필터링 지침 |
...
| [**`02_기능구현_프롬프트_예시.md`**](file:///c:/Users/oogg/Documents/Unreal%20Projects/FC/PromptExample/02_%EA%B8%B0%EB%8A%A5%EA%B5%AC%ED%98%84_%ED%94%84%EB%A1%AC%ED%94%84%ED%8A%B8_%EC%98%88%EC%8B%9C.md) | GAS 스킬, FastArray 인벤토리, 상호작용 컴포넌트, 전투 밸런스 튜닝용 실전 프롬프트 템플릿 |
